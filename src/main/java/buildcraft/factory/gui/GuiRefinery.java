...
public class GuiRefinery extends GuiAdvancedInterface {
    ...
    private static final ResourceLocation TEXTURE =
            new ResourceLocation("buildcraftfactory:textures/gui/refinery_filter.png");
        ...
        if (position >= 0 && position < 2) {
            if (k == 0) {
                if (!isShiftKeyDown()) {
                    ...
                    container.refinery.tankManager.get(position).colorRenderCache =
                            liquid.getFluid().getColor(liquid);
                } else {
    ...
    private void updateSlots() {
        ...
        if (crafting != null) {
            ...
            ((FluidSlot) slots.get(2)).colorRenderCache =
                    crafting.crafted.getFluid().getColor(crafting.crafted);
        } else {
